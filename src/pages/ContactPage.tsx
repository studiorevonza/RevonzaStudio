...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const fullMessage = `*New Inquiry from:* ${formData.name}%0A*Email:* ${formData.email}%0A*Phone:* ${formData.phone}%0A*Company:* ${formData.company || 'N/A'}%0A*Service Interested:* ${formData.service}%0A*Budget Range:* ${formData.budget || 'Not specified'}%0A*Timeline:* ${formData.timeline || 'Flexible'}%0A*Message:*%0A${encodeURIComponent(formData.message)}%0A%0A_Sent via Revonza Studio Contact Form_`;
